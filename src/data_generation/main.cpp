    ...
    for (uint nSeq = 0; nSeq < seq.size(); nSeq++) {
        ...
        for (uint f = 0; f < seqLength - 1; f++) {
            for (uint s = f+1; s < seqLength; s++) {
                ...
                sampledContour contF = dataBySeq[nSeq][f].sandContour;
                sampledContour contS = dataBySeq[nSeq][s].sandContour;
                ...
                if ((toolDist > 20) && (contDist > 20) && (contF[0].x > -1) && (contS[0].x > -1) && (leftBound)) {
...
void visualizeDataForLearningPush(const char * imagesFileName, const char * contoursFileName, const char * actionsFileName){
    ...
    for (uint i = 0; i < inAction.size() - 1; i++) {
        ...
        cout << "====== drawing data " << i << " ---> " << imageTriplets[i][0] << " " << imageTriplets[i][1] << " " << imageTriplets[i][2] << endl;
        ostringstream ossID;
